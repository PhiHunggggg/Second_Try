...
namespace Second_Try
{
    public partial class lichfrm : Form
    {
        ...
        {
            if(DateTimeBLL.Instance.CheckDate(dpk.Value) == false)
            {
            ...
            if(DateTimeBLL.Instance.CheckWeekend(dpk.Value) == true)
            {
                MessageBox.Show("Ngày được chọn không được là cuối tuần!");
                dpk.Value = DateTime.Now;
                return;
            }
            check = 1;
